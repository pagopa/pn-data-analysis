...
calcolo_riepilogo = report_penali.groupBy(
    ...
    F.sum(F.when((F.col("corrispettivo_penale_pesato") > 0), 1).otherwise(0)).alias(
    ...
    F.round(
        F.sum(
            F.col("corrispettivo_penale_pesato")
        ),  # fix: qui sommo direttamente il corrispettivo pesato
        2,

...
from pyspark.sql.functions import ceil, col, lit, udf, when
...
df1 = df1.withColumn(
    ...
    F.when(
        ...
        F.when(
            ...
            F.greatest(
                "demat_23l_ar_data_rendicontazione", "demat_plico_data_rendicontazione"
            )
        )
    ),
)
...
df1 = (
    df1.withColumn(
        ...
        F.when(
            ...
            + (
                F.when(
                    F.col("messaingiacenza_recapito_stato").isNotNull(), 1
                ).otherwise(0)
            )
            + (F.when(F.col("fine_recapito_stato").isNotNull(), 1).otherwise(0))
            + (
                F.when(
                    F.col("accettazione_23l_recag012_data").isNotNull(), 1
                ).otherwise(0)
            ),
        ).otherwise((F.when(F.col("fine_recapito_stato").isNotNull(), 1).otherwise(0))),
    )
    .withColumn(
        "esiti_plico",
        F.when(
    ...
    .withColumn("esiti_no_plico", F.col("tot_esiti") - F.col("esiti_plico"))
)
    ...
    F.when(
        ...
        F.when(
            (
                F.col("recapitista").isin(
                    "Poste", "FSU", "FSU - AR", "FSU - RS", "FSU - 890"
                )
            )
            & (
                F.col("affido_recapitista_con016_data")
                >= F.lit("2023-07-15 00:00:00").cast("timestamp")
            )
            & (
                F.col("affido_recapitista_con016_data")
                < F.lit("2024-05-01 00:00:00").cast("timestamp")
            ),
            F.when(
                F.to_date(F.col("tentativo_recapito_data_rendicontazione"))
                >= F.expr("date_add(to_date(tentativo_recapito_data), 16)"),
                (
                    (
                        F.unix_timestamp("tentativo_recapito_data_rendicontazione")
                        - F.unix_timestamp(
                            F.expr("date_add(to_date(tentativo_recapito_data), 16)")
                        )
                    )
            ...
            & (
                (
                    (
                        ...
                        >= F.lit("2024-06-01 00:00:00").cast("timestamp")
                    )
                    & (
                        F.col("accettazione_recapitista_con018_data")
                        < F.lit("2024-08-01 00:00:00").cast("timestamp")
                    )
                )
                | (
                    (
                        F.col("accettazione_recapitista_con018_data")
                        >= F.lit("2024-09-01 00:00:00").cast("timestamp")
                    )
                    & (
                        F.col("accettazione_recapitista_con018_data")
                        < F.lit("2024-10-01 00:00:00").cast("timestamp")
                    )
                )
            ),
            F.when(
                F.col("tentativo_recapito_data_rendicontazione").cast("date")
                >= F.expr("date_add(tentativo_recapito_data, 31)"),
                (
                    (
                        F.unix_timestamp("tentativo_recapito_data_rendicontazione")
                        - F.unix_timestamp(
                            F.expr("date_add(to_date(tentativo_recapito_data), 31)")
                        )
                    )
                    / 3600
                ).cast("int"),
        ...
        .when(
            ...
            F.when(
                ...
                & (
                    F.col("tentativo_recapito_data_rendicontazione").cast("date")
                    >= F.expr("date_add(tentativo_recapito_data, 6)")
                ),
                (
                    (
                        F.unix_timestamp("tentativo_recapito_data_rendicontazione")
                        - F.unix_timestamp(
                            F.expr("date_add(to_date(tentativo_recapito_data), 6)")
                        )
                    )
                    / 3600
                ).cast("int"),
            ...
            .when(
                ...
                & (
                    F.col("tentativo_recapito_data_rendicontazione").cast("date")
                    >= F.expr("date_add(tentativo_recapito_data, 8)")
                ),
                (
                    (
                        F.unix_timestamp("tentativo_recapito_data_rendicontazione")
                        - F.unix_timestamp(
                            F.expr("date_add(to_date(tentativo_recapito_data), 8)")
                        )
                    )
                    / 3600
                ).cast("int"),
        ...
        .when(
            ...
            F.when(
                ...
                & (
                    F.col("tentativo_recapito_data_rendicontazione").cast("date")
                    >= F.expr("date_add(tentativo_recapito_data, 11)")
                ),
                (
                    (
                        F.unix_timestamp("tentativo_recapito_data_rendicontazione")
                        - F.unix_timestamp(
                            F.expr("date_add(to_date(tentativo_recapito_data), 11)")
                        )
                    )
                    / 3600
                ).cast("int"),
            ...
            .when(
                ...
                & (
                    F.col("tentativo_recapito_data_rendicontazione").cast("date")
                    >= F.expr("date_add(tentativo_recapito_data, 16)")
                ),
                (
                    (
                        F.unix_timestamp("tentativo_recapito_data_rendicontazione")
...
df1 = df1.withColumn(
    ...
    F.when(
        ...
        F.when(
            F.to_date(F.col("messaingiacenza_recapito_data_rendicontazione"))
            >= F.expr("date_add(to_date(messaingiacenza_recapito_data), 16)"),
            (
                (
                    F.unix_timestamp("messaingiacenza_recapito_data_rendicontazione")
                    - F.unix_timestamp(
                        F.expr("date_add(to_date(messaingiacenza_recapito_data), 16)")
                    )
                )
    ...
    .when(
        ...
        F.when(
            F.to_date(F.col("messaingiacenza_recapito_data_rendicontazione"))
            >= F.expr("date_add(to_date(messaingiacenza_recapito_data), 31)"),
            (
                (
                    F.unix_timestamp("messaingiacenza_recapito_data_rendicontazione")
                    - F.unix_timestamp(
    ...
    .when(
        ...
        & (
            F.col("lotto").isin(
                "1", "3", "4", "6", "8", "12", "13", "14", "15", "17", "18", "19", "20"
            )
        ),
        F.when(
            F.to_date(F.col("messaingiacenza_recapito_data_rendicontazione"))
            >= F.expr("date_add(to_date(messaingiacenza_recapito_data), 6)"),
            (
                (
                    F.unix_timestamp("messaingiacenza_recapito_data_rendicontazione")
                    - F.unix_timestamp(
    ...
    .when(
        ...
        F.when(
            F.to_date(F.col("messaingiacenza_recapito_data_rendicontazione"))
            >= F.expr("date_add(to_date(messaingiacenza_recapito_data), 8)"),
            (
                (
                    F.unix_timestamp("messaingiacenza_recapito_data_rendicontazione")
                    - F.unix_timestamp(
    ...
    .when(
        ...
        F.when(
            F.to_date(F.col("messaingiacenza_recapito_data_rendicontazione"))
            >= F.expr("date_add(to_date(messaingiacenza_recapito_data), 11)"),
            (
                (
                    F.unix_timestamp("messaingiacenza_recapito_data_rendicontazione")
                    - F.unix_timestamp(
...
df1 = df1.withColumn(
    ...
    F.when(
        ...
        F.when(
            F.to_date(F.col("rend_23l_data_rendicontazione"))
            >= F.expr("date_add(to_date(accettazione_23l_recag012_data), 16)"),
            (
                (
                    F.unix_timestamp("rend_23l_data_rendicontazione")
                    - F.unix_timestamp(
                        F.expr("date_add(to_date(accettazione_23l_recag012_data), 16)")
                    )
                )
    ...
    .when(
        ...
        F.when(
            F.to_date(F.col("rend_23l_data_rendicontazione"))
            >= F.expr("date_add(to_date(accettazione_23l_recag012_data), 31)"),
            (
                (
                    F.unix_timestamp("rend_23l_data_rendicontazione")
                    - F.unix_timestamp(
    ...
    .when(
        ...
        F.when(
            F.to_date(F.col("rend_23l_data_rendicontazione"))
            >= F.expr("date_add(to_date(accettazione_23l_recag012_data), 6)"),
            (
                (
                    F.unix_timestamp("rend_23l_data_rendicontazione")
                    - F.unix_timestamp(
    ...
    .when(
        ...
        F.when(
            F.to_date(F.col("rend_23l_data_rendicontazione"))
            >= F.expr("date_add(to_date(accettazione_23l_recag012_data), 8)"),
            (
                (
                    F.unix_timestamp("rend_23l_data_rendicontazione")
                    - F.unix_timestamp(
    ...
    .when(
        ...
        F.when(
            F.to_date(F.col("rend_23l_data_rendicontazione"))
            >= F.expr("date_add(to_date(accettazione_23l_recag012_data), 11)"),
            (
                (
                    F.unix_timestamp("rend_23l_data_rendicontazione")
                    - F.unix_timestamp(
    ...
    F.when(
        ...
        F.when(
            ...
            F.when(
                ...
                F.when(
                    F.to_date(F.col("fine_recapito_data_rendicontazione"))
                    >= F.expr("date_add(to_date(certificazione_recapito_data), 16)"),
                    (
                        (
                            F.unix_timestamp("fine_recapito_data_rendicontazione")
                            - F.unix_timestamp(
                                F.expr(
                                    "date_add(to_date(certificazione_recapito_data), 16)"
                                )
                            )
                        )
                        / 3600
            ...
            .when(
                ...
                & (
                    (
                        ...
                        & (
                            F.col("accettazione_recapitista_con018_data")
                            < F.lit("2024-11-01 00:00:00").cast("timestamp")
                        )
                    )
                ),
                F.when(
                    F.to_date(F.col("fine_recapito_data_rendicontazione"))
                    >= F.expr("date_add(to_date(certificazione_recapito_data), 31)"),
                    (
                        (
                            F.unix_timestamp("fine_recapito_data_rendicontazione")
                            - F.unix_timestamp(
                                F.expr(
                                    "date_add(to_date(certificazione_recapito_data), 31)"
                                )
                            )
                        )
                        / 3600
            ...
            .when(
                ...
                F.when(
                    F.to_date(F.col("fine_recapito_data_rendicontazione"))
                    >= F.expr("date_add(to_date(certificazione_recapito_data), 6)"),
                    (
                        (
                            F.unix_timestamp("fine_recapito_data_rendicontazione")
                            - F.unix_timestamp(
                                F.expr(
                                    "date_add(to_date(certificazione_recapito_data), 6)"
                                )
                            )
                        )
                        / 3600
            ...
            .when(
                ...
                F.when(
                    F.to_date(F.col("fine_recapito_data_rendicontazione"))
                    >= F.expr("date_add(to_date(certificazione_recapito_data), 8)"),
                    (
                        (
                            F.unix_timestamp("fine_recapito_data_rendicontazione")
                            - F.unix_timestamp(
                                F.expr(
                                    "date_add(to_date(certificazione_recapito_data), 8)"
                                )
                            )
                        )
                        / 3600
            ...
            .when(
                ...
                F.when(
                    F.to_date(F.col("fine_recapito_data_rendicontazione"))
                    >= F.expr("date_add(to_date(certificazione_recapito_data), 11)"),
                    (
                        (
                            F.unix_timestamp("fine_recapito_data_rendicontazione")
                            - F.unix_timestamp(
                                F.expr(
                                    "date_add(to_date(certificazione_recapito_data), 11)"
                                )
                            )
                        )
                        / 3600
            ...
            .when(
                (F.col("prodotto") == "AR")
                & (
                    F.col("lotto").isin("26", "28", "29", "30BIS", "98")
                ),  # integrazione
                F.when(
                    F.to_date(F.col("fine_recapito_data_rendicontazione"))
                    >= F.expr("date_add(to_date(certificazione_recapito_data), 8)"),
            ...
            F.when(
                ...
                F.when(
                    F.to_date(F.col("dematerializzazione_data_rendicontazione"))
                    >= F.expr("date_add(to_date(certificazione_recapito_data), 16)"),
                    (
                        (
                            F.unix_timestamp("dematerializzazione_data_rendicontazione")
                            - F.unix_timestamp(
                                F.expr(
                                    "date_add(to_date(certificazione_recapito_data), 16)"
                                )
...
df1 = df1.withColumn(
    ...
    F.when(
        ...
        .when(
            F.to_date(F.col("dematerializzazione_data_rendicontazione"))
            >= F.expr("date_add(to_date(certificazione_recapito_data), 22)"),
            (
                (
                    F.unix_timestamp("dematerializzazione_data_rendicontazione")
                    - F.unix_timestamp(
...
spark.sql("""SELECT * FROM dettaglio""").writeTo(
    "send_dev.penali_rendicontazione_dettaglio"
...
SailpostData = (
    ...
    .agg(
        ...
        F.sum(F.coalesce(F.col("esiti_no_plico"), F.lit(0))).alias(
            "esiti_tot_no_plico"
        ),
        (
            F.sum(F.coalesce(F.col("esiti_plico"), F.lit(0)))
            + F.sum(F.coalesce(F.col("esiti_no_plico"), F.lit(0)))
        ).alias("esiti_tot"),
        F.sum(
            F.coalesce(F.col("esiti_rendicontati_violazione_sla_no_plico"), F.lit(0))
        ).alias("somma_esiti_violazione_no_plico"),
        F.sum(
            F.coalesce(F.col("esiti_rendicontati_violazione_sla_plico"), F.lit(0))
        ).alias("somma_esiti_violazione_plico"),
        F.floor(
            F.sum(
                F.coalesce(F.col("ritardo_tentativo_recapito"), F.lit(0))
                + F.coalesce(F.col("ritardo_messa_in_giacenza"), F.lit(0))
                + F.coalesce(F.col("ritardo_fine_recapito"), F.lit(0))
                + F.coalesce(F.col("ritardo_accettazione_23L"), F.lit(0))
            )  # /24
        ).alias("somma_ritardi"),
        F.floor(F.sum(F.coalesce(F.col("ritardo_plico"), F.lit(0))) / 24.0).alias(
            "ritardo_plico_in_giorni"
        ),
...
OtherRecapitistaData = (
    ...
    .agg(
        ...
        (
            F.sum(F.coalesce("esiti_plico", F.lit(0)))
            + F.sum(F.coalesce("esiti_no_plico", F.lit(0)))
        ).alias("esiti_tot"),
        F.sum(F.coalesce("esiti_rendicontati_violazione_sla_no_plico", F.lit(0))).alias(
            "somma_esiti_violazione_no_plico"
        ),
        F.sum(F.coalesce("esiti_rendicontati_violazione_sla_plico", F.lit(0))).alias(
        ...
        F.floor(
            (
                ...
                + F.sum(F.coalesce("ritardo_accettazione_23L", F.lit(0)))
            )  # / 24
        ).alias("somma_ritardi"),
        F.floor(F.sum(F.coalesce("ritardo_plico", F.lit(0))) / 24.0).alias(
            "ritardo_plico_in_giorni"
        ),
...
PenaleRendicontazioneSailpost = SailpostData.select(
    ...
    F.round(
        F.coalesce(
            ...
            / F.when(
                (F.col("esiti_tot_plico") + F.col("esiti_tot_no_plico")) == 0, None
    ...
    F.when(
        (
            F.col("somma_esiti_violazione_no_plico")
            / F.when(F.col("esiti_tot_no_plico") == 0, None).otherwise(
                F.col("esiti_tot_no_plico")
            )
        )
        - 0.021
        > 0,
    ...
    .otherwise(0)
    .alias("Penale_Rendicontazione_No_Plico"),
    F.when(
        (
            F.col("somma_esiti_violazione_plico")
            / F.when(F.col("esiti_tot_plico") == 0, None).otherwise(
                F.col("esiti_tot_plico")
            )
        )
        - 0.021
        > 0,
    ...
    .otherwise(0)
    .alias("Penale_Rendicontazione_Plico"),
    F.when(
        (
            (
...
PenaleRendicontazioneOther = OtherRecapitistaData.select(
    ...
    F.when(
        (
            (
                F.when(
                    F.col("somma_esiti_violazione_no_plico").isNull(), F.lit(0)
                ).otherwise(F.col("somma_esiti_violazione_no_plico"))
                / F.when(F.col("esiti_tot_no_plico") == 0, F.lit(None)).otherwise(
                    F.col("esiti_tot_no_plico")
                )
            )
            - 0.021
            ...
            F.round(
                F.greatest(
                    (
                        (
                            F.when(
                                F.col("somma_esiti_violazione_no_plico").isNull(),
                                F.lit(0),
                            ).otherwise(F.col("somma_esiti_violazione_no_plico"))
                            / F.when(
                                F.col("esiti_tot_no_plico") == 0, F.lit(None)
                            ).otherwise(F.col("esiti_tot_no_plico"))
                        )
                        - 0.021
                    )
                    * 1000
    ...
    F.when(
        (
            (
                ...
                / col("esiti_tot_plico").cast("double")
            ).alias("ratio")
            - 0.021
        )
        > 0,
    ...
    F.when(
        ...
        F.round(
            ...
            / (
                ...
                + F.coalesce(col("count_accettazione_23L"), lit(0))
            ).cast("double"),
            2,
        ),
    )
...
final_result = PenaleRendicontazione.select(
    ...
    ).alias("Esiti_con_Violazione_SLA"),
    "esiti_tot",
    "somma_esiti_violazione_no_plico",
    "esiti_tot_no_plico",
    F.round(
    ...
    F.round(F.coalesce(F.col("Ritardo_Medio") * 100, F.lit(0)), 2).alias(
        "Penale_Ritardo_Medio"
    ),
)
...
spark.sql("""SELECT * FROM aggregato""").writeTo(
    "send_dev.penali_rendicontazione_aggregato"
).using("iceberg").tableProperty("format-version", "2").tableProperty(
    "engine.hive.enabled", "true"
).createOrReplace()

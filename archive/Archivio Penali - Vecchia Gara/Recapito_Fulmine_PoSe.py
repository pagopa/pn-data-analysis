...
calcolo_tempo_recapito = calcolo_tempo_recapito.withColumn(
    ...
    F.when(
        ...
        .when( # Rilassamento lotto 22,27 Fulmine Campania - luglio 25 , nuovo rilassamento
            (F.col("lotto").isin(22,27)) & (F.col("regione").isin('Campania')) &
            (F.col("accettazione_recapitista_con018_data").between(F.lit('2025-07-01'), F.lit('2025-08-01'))), #vedere casi limite 7 luglio se è incluso
           45)
         .when( #  nuovo rilassamento parte II (prima settimana Agosto)
            (F.col("lotto").isin(22,27)) & (F.col("regione").isin('Campania')) &
            (F.col("accettazione_recapitista_con018_data").between(F.lit('2025-08-01'), F.lit('2025-08-08'))), 
           30)
          .when( #  nuovo rilassamento parte II (Novembre-Dicembre)
            (F.col("lotto").isin(22,27)) & (F.col("regione").isin('Campania')) &
            (F.col("accettazione_recapitista_con018_data").between(F.lit('2025-11-01'), F.lit('2025-12-31'))), 
           45) 
         .when( #  nuovo rilassamento parte III (Siracusa)
            (F.col("lotto").isin(22,27)) & (F.col("provincia").isin('Siracusa')) &
            (F.col("accettazione_recapitista_con018_data").between(F.lit('2025-06-30'), F.lit('2025-07-21')) |
            (F.col("accettazione_recapitista_con018_data").between(F.lit('2025-11-03'), F.lit('2025-11-09'))) |
            (F.col("accettazione_recapitista_con018_data").between(F.lit('2025-11-17'), F.lit('2025-11-23')))), 
           45)
         .when( #  nuovo rilassamento parte II (Messina)
            (F.col("lotto").isin(22,27)) & (F.col("provincia").isin('Messina')) &
            (F.col("accettazione_recapitista_con018_data").between(F.lit('2025-12-22'), F.lit('2025-12-28'))), 
           45)
         .when( #  nuovo rilassamento parte II (Palermo)
            (F.col("lotto").isin(22,27)) & (F.col("provincia").isin('Palermo')) &
            (F.col("accettazione_recapitista_con018_data").between(F.lit('2025-12-22'), F.lit('2025-12-28'))), 
           45)             
        .when(
            (F.col("lotto").isin(22, 27))

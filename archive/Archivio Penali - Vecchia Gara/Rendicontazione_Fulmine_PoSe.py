...
LEFT JOIN send_dev.cap_area_provincia_regione c ON (c.cap = g.geokey)
    ...
    AND CEIL(MONTH(fine_recapito_data_rendicontazione) / 3) = 1
...
record_count = spark.sql(
    "SELECT COUNT(*) AS total_records FROM gold_postalizzazione_analytics"
)
record_count.show()
...
df1 = df1.withColumn(
    ...
    F.when(
            ...
            F.when(
                (
                    datediff_workdays_udf(
                        ...
                        F.col("tentativo_recapito_data_rendicontazione"),
                    )
                    - (30 * 24)
                )
                > 0,
        ...
        .when(  ### Rilassamento Campania
            ...
            & (
                (
                    (
                        F.col("accettazione_recapitista_con018_data")
                        >= F.lit("2025-06-01 00:00:00").cast("timestamp")
                    )
                    & (
                        F.col("accettazione_recapitista_con018_data")
                        < F.lit("2025-08-08 00:00:00").cast("timestamp")
                    )
                )
            ),
            F.when(
        ...
        .when(  ### Rilassamento Siracusa
            (F.col("lotto").isin(27, 22))
            & (F.col("provincia").isin("Siracusa"))
            & (
                (
                    (
                        F.col("accettazione_recapitista_con018_data")
                        >= F.lit("2025-06-30 00:00:00").cast("timestamp")
                    )
                    & (
                        F.col("accettazione_recapitista_con018_data")
                        < F.lit("2025-07-21 00:00:00").cast("timestamp")
                    )
                )
            ),
            F.when(
...
df1 = df1.withColumn(
    ...
    .when(
        ...
        F.when(
            (
                datediff_workdays_udf(
                    F.col("messaingiacenza_recapito_data"),
                    F.col("messaingiacenza_recapito_data_rendicontazione"),
                )
                - (30 * 24)
            )
            > 0,
...
df1 = df1.withColumn(
    ...
    .when(
        ...
        F.when(
            (
                datediff_workdays_udf(
                    F.col("accettazione_23l_recag012_data"),
                    F.col("rend_23l_data_rendicontazione"),
                )
                - (30 * 24)
            )
            > 0,
        ...
        .when(  ### Rilassamento Siracusa
            ...
            F.when(
                (
                    datediff_workdays_udf(
                        F.col("certificazione_recapito_data"),
                        F.col("certificazione_recapito_data_rendicontazione"),
                    )
                    - (10 * 24)
                )
                > 0,

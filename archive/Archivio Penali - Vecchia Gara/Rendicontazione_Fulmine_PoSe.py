    ...
    F.when(F.col("tentativo_recapito_stato").isin('RECRN010', 'RECRS010', 'RECAG010'),
            F.when(
                ...
                (
                    ...
                    (F.col("accettazione_recapitista_con018_data") < F.lit("2025-08-08 00:00:00").cast("timestamp"))) 
                ),
                F.when(
                ...
                (F.col("lotto").isin(27, 22)) & (F.col("provincia").isin('Siracusa')) &
                (
                    ((F.col("accettazione_recapitista_con018_data") >= F.lit("2025-06-30 00:00:00").cast("timestamp")) &
                    (F.col("accettazione_recapitista_con018_data") < F.lit("2025-07-21 00:00:00").cast("timestamp"))) 
                ),
                F.when(
                    (datediff_workdays_udf(F.col("tentativo_recapito_data"),F.col("tentativo_recapito_data_rendicontazione")) - (10*24)) > 0,
                    (datediff_workdays_udf(F.col("tentativo_recapito_data"),F.col("tentativo_recapito_data_rendicontazione")) - (10*24)).cast("int")
    ...
    F.when(
        ...
        F.when(
            ...
            (datediff_workdays_udf(F.col("accettazione_23l_recag012_data"),F.col("rend_23l_data_rendicontazione")) - (10*24)).cast("int")
                ...
                F.when(
                    (datediff_workdays_udf(F.col("accettazione_23l_recag012_data"),F.col("accettazione_23l_recag012_data_rendicontazione")) - (10*24)) > 0,
                    (datediff_workdays_udf(F.col("accettazione_23l_recag012_data"),F.col("accettazione_23l_recag012_data_rendicontazione")) - (10*24)).cast("int")
                ).otherwise(None)
            ).when(
        (F.col("prodotto") == 890) & (F.col("lotto").isin(1, 3, 4, 6, 8, 12, 13, 14, 15, 17, 18, 19, 20)),
        F.when(
    ...
    F.when(
        ...
        F.when(
            ...
            F.when(
                ...
                F.when(
                    (datediff_workdays_udf(F.col("certificazione_recapito_data"),F.col("certificazione_recapito_data_rendicontazione")) - (10*24)) > 0,
                    (datediff_workdays_udf(F.col("certificazione_recapito_data"),F.col("certificazione_recapito_data_rendicontazione")) - (10*24)).cast("int")
                ).otherwise(None)
            ).when(
            (F.col("prodotto") == 890) & (F.col("lotto").isin(1, 3, 4, 6, 8, 12, 13, 14, 15, 17, 18, 19, 20)),
            F.when(

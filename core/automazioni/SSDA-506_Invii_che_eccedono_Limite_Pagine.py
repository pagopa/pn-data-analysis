...
SHEET_ID = "1Ts6LJ0P4kt-K7t5k6HF4YxhrzvYoWrsAKvOTRdCXD1o" #trovare il modo di stampare su terminale il percorso del file
...
def run_query(spark: SparkSession) -> pd.DataFrame:
    ...
    query_sql = """
        SELECT
            ...
            n.subject,
            n.taxonomycode,
            get_json_object(t.details, '$.physicalAddress.zip') cap,
            CAST(get_json_object(t.details, '$.numberOfPages') as bigint) pages,
            CAST(get_json_object(t.details, '$.envelopeWeight') as bigint) weight
        ...
        WHERE
            t.category = 'SEND_ANALOG_DOMICILE'
            AND cancelled.iun is null
            AND viewed.iun is null
            AND (
            (CAST(get_json_object(t.details, '$.numberOfPages') as bigint) > 18 AND n.physicalcommunicationtype = 'REGISTERED_LETTER_890')
            OR (CAST(get_json_object(t.details, '$.numberOfPages') as bigint) > 100 AND n.physicalcommunicationtype = 'AR_REGISTERED_LETTER')
            )
        """
...
def main():
    ...
    spark = SparkSession.builder.appName("SSDA-506 Invii che Eccedono il Limite di Pagine").getOrCreate()
    ...
    sheet_name = "Estrazione weekly SSDA-506"

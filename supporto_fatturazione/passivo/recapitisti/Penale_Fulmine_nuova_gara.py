...
calcolo_penale_rendicontazione = calcolo_penale_rendicontazione.withColumn(
    ...
    F.col("corrispettivo_a_capienza_60gg_CostoRec_Recapito")
    + F.col("corrispettivo_a_capienza_60gg_CostoRec_Rendicontazione"),
)
...
calcolo_riepilogo_globale = calcolo_penale_rendicontazione.groupBy(
    ...
    F.when(
        ...
        F.year(
            F.to_date(
                F.substring(F.col("fine_recapito_data_rendicontazione"), 1, 10),
                "yyyy-MM-dd",
            )
        ),
    ).alias("Anno"),

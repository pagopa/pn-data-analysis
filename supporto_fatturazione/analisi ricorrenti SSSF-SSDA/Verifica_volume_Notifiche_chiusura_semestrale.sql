-- Verifica volume Notifiche per chiusura semestrale
-- Sostituire solo start_periodo con il primo giorno del semestre

WITH parametri_raw AS (
    SELECT
        CAST('2025-07-01' AS DATE) AS start_periodo
),
parametri AS (
    SELECT
        CAST(start_periodo AS STRING) AS start_periodo,
        CAST(ADD_MONTHS(start_periodo, 4) AS STRING) AS start_ultimi_2_mesi,
        CAST(ADD_MONTHS(start_periodo, 6) AS STRING) AS end_periodo,
        CAST(DATE_SUB(ADD_MONTHS(start_periodo, 6), 1) AS STRING) AS end_periodo_label
    FROM parametri_raw
),
base_notifiche AS (
    SELECT
...
SELECT
    ...
    COUNT(CASE
        ...
        THEN c.iun
    END) AS `Non Perfezionate nel semestre`,
    --
    CONCAT(
        '[',
        p.start_periodo,

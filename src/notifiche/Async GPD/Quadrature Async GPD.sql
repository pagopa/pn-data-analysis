...
WITH asyncNotification AS (
  SELECT a.iun, 
    a.recipients_size > 1 as multidestinatario,
    pagopaintmode,
    r.pos as recindex,p.creditorTaxId, 
    p.noticecode, 
    p.applyCost, 
  ...
  FROM asyncNotification a LEFT JOIN silver_notification n ON a.iun = n.iun

...
df = spark.table("send.gold_notification_analytics").filter(F.col("pagopaintmode") == "ASYNC")
...
spark.sql("""SELECT * FROM DF_OUTPUT""").writeTo("send_dev.temp_quadrature_gpd")\
                .using("iceberg")\
...
############### Estrazione aggregato - scrittura in tabella 
spark.sql("""SELECT * FROM DF_UNMATCHED""").writeTo("send_dev.temp_quadrature_gpd_unmatched")\
                .using("iceberg")\
                .tableProperty("format-version","2")\
                .tableProperty("engine.hive.enabled","true")\
                .createOrReplace()

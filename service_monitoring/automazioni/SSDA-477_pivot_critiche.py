...
def run_query(spark: SparkSession) -> dict:
    ...
    df_completa_pandas = df_completa_spark.toPandas()
    df_senza_chiuse_pandas = df_senza_chiuse_spark.toPandas()

    # gestione delle casistiche NaN/NaT
    df_completa_pandas = sanitize_for_sheets(df_completa_pandas, "-")
    df_senza_chiuse_pandas = sanitize_for_sheets(df_senza_chiuse_pandas, "-")

    logging.info("Trasformazione in Pandas DataFrame...")

    return {
        "pivot_completa": df_completa_pandas,
        "pivot_senza_chiuse": df_senza_chiuse_pandas,
    }
...
def main():
    ...
    if not df_input.empty:
        colonna_id = df_input.columns[
            0
        ]  # devo prendere la prima colonna del file contenente i requestid
        ids_input = (
            df_input[colonna_id].dropna().astype(str).str.strip().unique().tolist()
        )

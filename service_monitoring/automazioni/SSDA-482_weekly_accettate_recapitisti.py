...
# VERSIONE 2: FIX SULLA GESTIONE DEI NULL
...
def run_query(spark: SparkSession) -> dict:
    ...
    query_sql_base_Sailpost = """
        temp AS (
            SELECT
                ...
                CASE WHEN lotto = '30' AND region_name NOT IN ('Puglia') THEN 1
                    ELSE 0
                END AS flag_30bis,
    ...
    vista_aggregata_lotto = """
        SELECT
            ...
            COUNT(*) AS totale
        FROM temp t
        WHERE t.affido_accettazione_rec_data >= '2025-01-01'
        AND t.statusrequest NOT IN ('PN999', 'PN998')
        AND flag_30bis = 0
    ...
    vista_aggregata_lotto_anno_mese = """
        SELECT
            t.lotto,
            SUBSTRING(CAST(t.affido_accettazione_rec_data AS STRING), 1, 7) AS mese_anno_accettazione,
            SUM(CASE WHEN t.fine_recapito_stato IS NOT NULL THEN 1 ELSE 0 END) AS completate,
            SUM(CASE WHEN t.fine_recapito_stato IS NULL AND t.flag_wi7_report_postalizzazioni_incomplete = 0 THEN 1 ELSE 0 END) AS in_corso,
            SUM(CASE WHEN t.fine_recapito_stato IS NULL AND t.flag_wi7_report_postalizzazioni_incomplete = 1 THEN 1 ELSE 0 END) AS incomplete,
            COUNT(*) AS totale
        ...
        AND flag_30bis = 0
        GROUP BY GROUPING SETS (
            (t.lotto, SUBSTRING(CAST(t.affido_accettazione_rec_data AS STRING), 1, 7)), -- dettaglio
            (t.lotto),  -- subtotale per lotto
            ()          -- totale generale
        )
        ORDER BY
    ...
    # Query di Sailpost
    aggregato_Sailpost_lotto = (
        dati_gold_corretti + query_sql_base_Sailpost + vista_aggregata_lotto
    )
    aggregato_Sailpost_lotto_regione = (
        dati_gold_corretti + query_sql_base_Sailpost + vista_aggregata_regione
    )
    # Query di POST & SERVICE
    aggregato_PoSe_lotto_anno_mese = (
        dati_gold_corretti + query_sql_base_PoSe + vista_aggregata_lotto_anno_mese
    )
    aggregato_PoSe_lotto_regione = (
        dati_gold_corretti + query_sql_base_PoSe + vista_aggregata_regione
    )
    ...
    df_sailpost_lotto = sanitize_for_sheets(df_sailpost_lotto, "-")
    df_sailpost_regione = sanitize_for_sheets(df_sailpost_regione, "-")
    df_pose_lotto_mese = sanitize_for_sheets(df_pose_lotto_mese, "-")
    df_pose_regione = sanitize_for_sheets(df_pose_regione, "-")
...
def main():
    ...
    spark = SparkSession.builder.appName(
        "SSDA-482 Estrazione Accettate Recapitisti"
    ).getOrCreate()
    ...
    export_to_sheets(
        results["sailpost_lotto"], creds, SHEET_ID_SAILPOST, "Sailpost - Lotto"
    )
    ...
    export_to_sheets(
        results["pose_lotto_anno_mese"], creds, SHEET_ID_POSE, "PoSe - Lotto Mese"
    )

    export_to_sheets(
        results["pose_lotto_regione"], creds, SHEET_ID_POSE, "PoSe - Regione"
    )
    ...
    sheet_sail = Sheet(
        sheet_id=SHEET_ID_SAILPOST, service_credentials=creds, id_mode="key"
    )

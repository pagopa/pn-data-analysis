        ...
        SELECT
            ...
            ROW_NUMBER() OVER (
                PARTITION BY
                    ...
                    END
                ORDER BY
                    e.paperprogrstatus.clientrequesttimestamp DESC
            ) AS rn
        FROM
    ...
    finale AS (
        SELECT
            ...
            CONCAT_WS (
              ...
              -- CASE WHEN controllo_causale = 'OK' THEN NULL ELSE controllo_causale END AS controllo_causale,
              -- CASE WHEN controllo_date_business = 'OK' THEN NULL ELSE controllo_date_business END AS controllo_date_business,
              -- CASE WHEN controllo_tripletta = 'OK' THEN NULL ELSE controllo_tripletta END AS controllo_tripletta,
              -- CASE WHEN controllo_tempistiche_compiuta_giacenza = 'OK' THEN NULL ELSE controllo_tempistiche_compiuta_giacenza END AS controllo_tempistiche_compiuta_giacenza,
              -- CASE WHEN controllo_inesito_casi_giacenza = 'OK' THEN NULL ELSE controllo_inesito_casi_giacenza END AS controllo_inesito_casi_giacenza,
               t.controllo_causale,
...
---------------------------------------------------------------------------------------------------------
--- VGeneral: aggiungere il senderdenomination partendo dalla silver_notification --> left join alla fine -- in scrittura sul seguente file: ID_GENERAL:

---------------------------------------------------------------------------------------------------------
--- VRecapitisti: quali campi portarci dietro? 
--- fare un file per ogni recapitista ID_FULMINE: | ID_PO_SE: | ID_POSTE: | ID_SAILPOST:
...
---------------------------------------------------------------------------------------------------------
--- VL3
-- in scrittura sul seguente file: ID_L3
SELECT DISTINCT
   senderpaid,
